...
### Task 5b: sample.ts를 실제 구글 모양으로 되돌리기

**Files:**
- Modify: `src/app/playground/together-map/sample.ts`
- Modify: `src/app/playground/together-map/sample.test.ts`

**왜**

Task 5에서 방문 구간 문제(Task 4c 참고)에 부딪힌 구현자가 **샘플 쪽을 바꿔서** 빠져나갔다 —
머문 구간을 10~20분마다 새 방문 세그먼트로 쪼갰다. 구글은 그렇게 뱉지 않는다.
한 번의 머묾은 세그먼트 하나이고, 시작·끝 시각과 좌표 하나를 갖는다.

Task 4c가 `until`을 넣어 근본 원인을 고쳤으니, 이제 우회를 걷어내야 한다.
**진짜 파일이 없는 상태에서 이 샘플이 유일한 대역이라, 실제와 다르면 아래 모든 것이
스스로에게 유리한 허구 위에서 검증된다.**

**할 일**

1. **방문 쪼개기를 되돌린다.** 연속한 방문 샘플을 하나의 세그먼트로 묶고,
   `startTime`은 처음, `endTime`은 마지막으로 적는다. 좌표는 하나다.
   Task 4c가 `endTime`을 읽으므로 이제 제대로 동작한다.

2. **`activity`만 있는 세그먼트를 넣는다.** `parse.ts`는 `activity.start`/`end`를
   읽는 길을 갖고 있는데 샘플이 그 길을 한 번도 안 밟는다. 이동 구간 일부를
   `timelinePath` 없이 `activity`만으로 내보낸다 — 실제 파일에도 둘 다 있다.
   안드로이드는 거리를 숫자로, 아이폰은 문자열로 적는 차이까지 살린다.

3. **구멍과 튄 점이 살아 있는지 테스트한다.** 지금은 그 주입 코드를 통째로 지워도
   테스트가 전부 통과한다. 파싱된 결과에서 30분 넘는 구멍이 실제로 존재하는지,
   비현실적 속도로 튄 점이 실제로 존재하는지 각각 확인한다.
   이게 없으면 필터와 구멍 처리에 물릴 것이 없는 샘플이 조용히 만들어진다.

4. **행동 기준을 다시 확인한다.** 만남 5건 이상 60건 미만, 제주 0건, 부산 1건 이상.
   방문 모양이 바뀌었으니 숫자가 달라진다. **기준을 고치지 말고 생성기를 맞춘다.**

- [ ] **Step 1: 3번 테스트를 먼저 쓴다 — 지금 코드에서 통과하는지 확인한다.**
      통과하면 그게 문제다(주입 코드를 지워도 통과한다는 뜻). 실패해야 정상이다.
- [ ] **Step 2: 방문 묶기를 되돌리고 전체를 돌린다**
- [ ] **Step 3: activity 전용 세그먼트를 넣는다**
- [ ] **Step 4: 행동 기준에 맞게 생성기를 조정한다**
- [ ] **Step 5: 변이 확인 — 주입 코드를 지우면 3번 테스트가 죽는가**
- [ ] **Step 6: 커밋**
