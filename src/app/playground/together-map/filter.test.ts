...
describe("이상치 필터 (보수적)", () => {
  ...
  it("같은 시각의 점 양쪽에서 만나면 버린다 — 깨진 기록이다", () => {
    // 같은 순간(0분)에 세 점(같은 위치), 그 다음 1분에 네 번째 점
    // 두 번째 점은 앞에서도(0→0) 뒤에서도(0→0) 무한 속도라 버려진다
    const points = [
      p(0, 37.5, 127.0),
      p(0, 37.5, 127.0), // 앞뒤 모두 시간 0 — 버려진다
      p(0, 37.5, 127.0),
      p(1, 37.5, 127.0),
    ];
    const kept = filterPoints(points, ON);
  ...
  it("같은 시각의 점이 한쪽에만 있으면 남긴다 — 한쪽만 비현실적이면 통과", () => {
    // 0분에 두 점(같은 위치), 0분에 또 다른 점, 1분에 네 번째 점
    // 두 번째 점은 앞에서만(0→0) 무한, 뒤에서는(0→0) 무한이므로 버려짐
    // 세 번째 점은 앞에서만(0→0) 무한, 뒤에서는(0→1) 정상이므로 남음
    const points = [
      p(0, 37.5, 127.0),
      p(0, 37.5, 127.0), // 양쪽 모두 무한 — 버려진다
      p(0, 37.5, 127.0), // 한쪽만 무한 — 남는다
      p(1, 37.5, 127.0),
    ];
    const kept = filterPoints(points, ON);
    expect(kept).toHaveLength(3);
