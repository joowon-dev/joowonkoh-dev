 ...
 * 수직 화각.
 *
 * 55°로 시작했다가 60°로 넓혔다. 55°에서는 가로 화각이 ±42.8°라 1루와 3루가
 * 딱 2° 차이로 화면 밖에 남는다 — 베이스도 베이스패스도 영영 안 보인다는 뜻이다.
 * 60°면 16:9에서 가로 ±45.8°가 되어 다이아몬드가 겨우 들어온다.
 *
 * 대가는 공이 13% 작아지는 것이다. 넓히면 다 보이는 대신 주인공이 줄어든다.
 */
export const FIELD_OF_VIEW = (60 * Math.PI) / 180;
/** 이보다 가까워지면 투영이 발산한다 */
const NEAR = 0.05;
