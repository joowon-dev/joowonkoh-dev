import { notFound } from "next/navigation";
import type { Metadata } from "next";
import SectionPostView from "@/components/SectionPostView";
import ArticleJsonLd from "@/components/ArticleJsonLd";
import {
  getAllPostsBySection,
  getPostBySlugFromSection,
} from "@/lib/mdx";

interface Props {
  params: Promise<{ slug: string }>;
}

export async function generateStaticParams() {
  const posts = getAllPostsBySection("life");
  return posts.map((post) => ({ slug: post.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  try {
    const { meta } = getPostBySlugFromSection("life", slug);
    const url = `https://joowonkoh.com/blog/life/${slug}`;
    return {
      title: meta.title,
      description: meta.description,
      openGraph: {
        type: "article",
        title: meta.title,
        description: meta.description,
        url,
        siteName: "Joowon Koh",
        locale: "ko_KR",
        publishedTime: meta.date,
        authors: ["고주원"],
        images: [{ url: "/logo.png", width: 512, height: 512, alt: meta.title }],
      },
      twitter: {
        card: "summary",
        title: meta.title,
        description: meta.description,
        images: ["/logo.png"],
      },
      alternates: { canonical: url },
      robots: meta.noindex ? { index: false, follow: true } : undefined,
    };
  } catch {
    return { title: "Not Found" };
  }
}

export default async function LifePostPage({ params }: Props) {
  const { slug } = await params;

  let post: ReturnType<typeof getPostBySlugFromSection>;
  try {
    post = getPostBySlugFromSection("life", slug);
  } catch {
    notFound();
  }

  const allPosts = getAllPostsBySection("life");
  const currentIndex = allPosts.findIndex((p) => p.slug === slug);
  const prev =
    currentIndex < allPosts.length - 1 ? allPosts[currentIndex + 1] : null;
  const next = currentIndex > 0 ? allPosts[currentIndex - 1] : null;

  return (
    <>
      <ArticleJsonLd meta={post!.meta} />
      <SectionPostView
        meta={post!.meta}
        content={post!.content}
        prev={prev}
        next={next}
      />
    </>
  );
}
