...
description: >-
  블로그 글 4개를 추가했더니 레포가 178MB가 됐다. sharp + WebP + 자동 리사이즈 스크립트로 8.5MB까지 줄였다. 이미지
  워크플로우와 빌드 통합 방법까지.
date: '2026-04-21'
tags:
  - 이미지
...
## 직접 겪은 일: 파일명을 안 바꿔서 옛 사진이 계속 나왔다

찜질방 페이지에 방 사진을 여러 장 깔아뒀는데, 습식사우나 사진이 처음에는 청록색이었다. 습기를 물로 읽어 파랗게 잡았더니, 28~35도짜리 더운 방이 화면에서는 제일 시원해 보였다. 그래서 따뜻한 색으로 다시 만들어 같은 경로에 덮어썼다.

배포하고 열어보니 여전히 청록색이었다. 새로고침해도 그대로였다.

주소가 같았기 때문이다. 정적 파일에 4시간짜리 캐시가 걸려 있어서, 브라우저와 CDN 양쪽이 이미 받아둔 옛 사진을 그대로 들고 있었다. 내용이 바뀐 걸 알아낼 방법이 없다 — 주소가 같으면 같은 파일이다.

`wetSauna.webp`를 `wetSauna-warm.webp`로 바꾸고 나서야 새 사진이 나왔다.

**압축 설정을 바꿔 이미지를 다시 뽑을 때는 파일명도 같이 바꿔야 한다.** 화질만 조정해 같은 이름으로 덮어쓰는 경우가 특히 위험하다. 눈에 띄게 달라졌는데 화면은 그대로라면, 압축 스크립트보다 캐시를 먼저 의심하는 편이 빠르다.
