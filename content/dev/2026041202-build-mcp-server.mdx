...
![직접 만든 MCP 서버를 클라이언트에 연결하는 구조를 나타낸 일러스트](/blog/2026041202-build-mcp-server/hero.jpg)
...
## 직접 겪은 일: 결국 붙어 있는 MCP 서버는 하나뿐이다

만드는 과정을 다 적어놨지만, 몇 달 지난 지금 내 설정에 실제로 붙어 있는 MCP 서버는 **하나**다. Obsidian 하나.

처음엔 이것저것 붙였다. 그런데 붙여만 놓고 안 쓰는 서버는 손해가 분명하다. **도구 목록이 길어지면 매 요청마다 그만큼의 설명이 같이 실려 나간다.** 안 쓰는 서버는 조용히 비용만 만든다.

살아남은 하나의 공통점을 생각해 보면 이렇다. 그 서버가 다루는 데이터가 **내 컴퓨터 안에 있고, 다른 방법으로는 접근이 번거로운 것**이었다. Obsidian 보관함이 그렇다. 웹에서 받아올 수 있는 것들은 결국 다른 방법으로 대체됐다.

MCP 서버를 만들어 볼 가치는 충분하다. 만들어 보면 프로토콜이 어떻게 생겼는지 알게 된다. 다만 **만든 것과 계속 붙여둘 것은 다른 판단**이라는 걸 나중에 알았다.
