...
description: >-
  next-mdx-remote로 MDX 파싱, gray-matter로 frontmatter 추출, rehype-pretty-code로 코드
  하이라이팅을 구현한 과정입니다.
date: '2026-04-04'
tags:
  - 도구
...
![MDX 파일이 파서를 거쳐 웹 페이지로 바뀌는 과정을 나타낸 일러스트](/blog/2026040103-building-mdx-blog-system/hero.jpg)
...
파일명 앞의 숫자를 잘라 주소로 쓰기로 했다. `2026040103-building-mdx-blog-system.mdx`면 주소는 `/blog/2026040103`이 되는 식이다. 짧고 안 겹쳐서 좋아 보였다.
...
되돌리는 방법은 옛 주소를 새 주소로 잇는 것뿐이었다. 지금 `next.config.ts`가 77줄인데 그중 상당 부분이 이 리디렉션을 만드는 코드다. 손으로 적지 않고 `content/` 폴더를 읽어서 만들게 했다 — 어느 글이 dev고 어느 글이 life인지는 폴더가 알고 있으니까.
