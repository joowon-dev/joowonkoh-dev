...
## 직접 겪은 일: 이 블로그의 PR 서른여섯 개를 이렇게 처리했다

이 레포에 지금까지 머지된 PR이 서른여섯 개다. 그중 브라우저를 열고 처리한 건 손에 꼽는다.

흐름은 거의 고정돼 있다. `gh pr create`로 올리고, `gh pr view --json mergeable,statusCheckRollup`으로 체크가 초록인지 보고, `gh pr merge`로 닫는다.

여기서 실제로 값을 한 건 두 번째 단계였다. 브라우저로 볼 때는 **체크가 아직 도는 중인지 실패했는지를 눈으로 구분하려다 자꾸 새로고침**을 했다. `--json`으로 뽑으면 상태가 글자로 나온다. `MERGEABLE`인지 `BLOCKED`인지가 한 줄로 끝난다.

반대로 터미널로 안 되는 것도 분명하다. 남이 남긴 리뷰 코멘트가 코드 어느 줄에 붙었는지 따라가는 일은 아직 브라우저가 낫다. 혼자 하는 레포라 그럴 일이 없어서 여태 안 아쉬웠을 뿐이다.

**gh CLI의 값은 «브라우저를 안 쓴다»가 아니라 «상태를 눈이 아니라 값으로 확인한다»는 쪽**에 있었다.
