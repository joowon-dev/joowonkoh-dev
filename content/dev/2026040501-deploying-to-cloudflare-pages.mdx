...
![빌드 결과물이 전 세계 엣지 서버로 배포되는 과정을 나타낸 일러스트](/blog/2026040501-deploying-to-cloudflare-pages/hero.jpg)
...
`package.json`의 배포 스크립트는 이렇게 생겼다.
...
Cloudflare에 올리는데 폴더 이름이 `.vercel`이다. 처음 봤을 때 설정을 잘못한 줄 알고 한참 뒤졌다. 알고 보니 `next-on-pages`가 Vercel의 빌드 출력 규격을 중간 형식으로 쓰기 때문이다. 잘못된 게 아니라 원래 그렇다.

`next.config.ts`에는 `output: "standalone"`이 들어 있다. 이것도 Cloudflare용이라기보다 «Node 서버 하나로 묶어라»에 가까운 설정이라 이름만 봐서는 연결이 안 된다.
