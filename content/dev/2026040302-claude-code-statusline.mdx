...
![터미널 하단 상태 표시줄의 구성 요소를 나타낸 일러스트](/blog/2026040302-claude-code-statusline/hero.jpg)
...
추천 구성을 적어놨지만 내 것은 그보다 훨씬 길어졌다. 지금 `statusline-command.sh`는 164줄이고, 들어온 JSON에서 값을 꺼내는 `jq` 호출만 13번이다.

읽는 것들은 이렇다. 모델 이름, 현재 디렉터리, 컨텍스트 사용률, 5시간·7일 사용량, vim 모드, git 브랜치, worktree 이름과 브랜치, 서브에이전트 이름, 출력 스타일, 세션 이름, 입출력 토큰 총량.

처음부터 이만큼 넣은 게 아니다. **한 줄씩 붙은 계기가 전부 «몰라서 당한 일»이었다.** 컨텍스트 사용률은 갑자기 압축이 걸려서 붙였고, 5시간 사용량은 한도에 걸린 다음에 붙였고, worktree 브랜치는 엉뚱한 브랜치에 커밋한 뒤에 붙었다.

그래서 추천 구성이라는 게 사실 별 의미가 없다. 상태 표시줄은 **자기가 데인 자리의 기록**이라, 남의 것을 그대로 가져다 쓰면 아직 데어보지 않은 항목만 잔뜩 보게 된다.
