...
description: >-
  마크다운 표는 모바일에서 깨진다. MDX의 진짜 가치는 표를 시각화 컴포넌트로 대체하는 데 있다. CompareCard, PlanCard,
  FlowCard 같은 시각화 컴포넌트의 설계 원칙을 정리한다.
date: '2026-04-22'
tags:
  - MDX
  - 디자인
...
![모바일에서 깨지는 표를 카드형 컴포넌트로 대체한 모습의 일러스트](/blog/2026042201-mdx-table-replacement-with-react-components/hero.jpg)
...
`bg-zinc-900`, `text-emerald-400`처럼 팔레트 색을 그대로 쓰고 싶어지는데, 여기서 참아야 한다. 시각화 컴포넌트는 **테마 토큰만 쓴다.**
...
이유는 나중에 드러난다. 테마를 한 번이라도 손보면 색을 직접 박은 컴포넌트만 혼자 남는다. 토큰으로 쓰면 `globals.css` 한 곳만 고치면 따라온다. [단일 테마 디자인 글](/blog/dev/2026042202)에서 그 토큰 구성을 다뤘다.
...
이 패턴을 다 만들어 놓고, 한동안 글 열한 편에서 카드가 안 보이고 있었다. 에러도 안 났고 빌드도 통과했다. 그래서 한참을 몰랐다.

범인은 `next-mdx-remote`였다. 6버전부터 `blockJS`가 기본으로 켜지는데, 이게 MDX 안의 JS 표현식을 전부 걷어낸다. 그러면 이런 코드가

```mdx
<DataTableCard rows={[{ name: "A", value: 1 }]} />
```

`rows`만 사라진 채로 남는다. 컴포넌트는 정상적으로 호출되고 props만 비어 있다. 빈 배열을 받은 카드는 아무것도 그리지 않고 조용히 자리를 비운다. 던질 에러가 없으니 아무 신호도 오지 않는다.
...
이 옵션은 남이 쓴 MDX를 렌더할 때를 위한 것이다. 여기서 읽는 건 내가 쓴 `content/` 폴더뿐이라 꺼도 된다. 위험한 호출을 막는 `blockDangerousJS`는 기본값 그대로 켜 뒀다.

교훈은 이거다. **props를 JS 표현식으로 넘기는 컴포넌트를 MDX에서 쓸 거면, 배포한 뒤 실제 페이지를 한 번은 열어봐야 한다.** 빌드 초록불이 이 경우에는 아무것도 보장해주지 않았다.
